...
export default function KabnabTaskInProgress({
  tasks,
}: {
  tasks: Task[];
}) {
  const { ref } = useDroppable({ id: "in-progress-droppable" });
  return (
    <>
      <ScrollArea className="relative w-full h-87.5 border rounded-xl">
        ...
        <div className="w-full z-5 h-10 bg-accent rounded-t-md absolute top-0">
          <h3 className="text-center p-2 ">Tasks In-Progress</h3>
        </div>
        <div className="z-3 relative px-4 mt-14">
          {tasks.map((item, index) => {
            if(TaskStatus[item.status] === "IN_PROGRESS")
